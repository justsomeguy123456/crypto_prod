...
for c in coin:
    ...
    for t in trades:
        ...
        if t['commissionAsset'] == 'USD':
            ...
            trade_dict['commission_val'].append(  float(t['quoteQty']) * .00075)
...
df1['symbol'] = 'BNB'
df1['price'] = ((df1['quoteQty']*.00075) / pd.to_numeric(df1['commission'])).abs()
...
for w in withdraws:
    ...
    deposit_dict['orderListId'] = ''
    deposit_dict['price'] = 0
    deposit_dict['qty'].append(float(w['amount'])*-1.000000000000 + float(w['transactionFee']) * -1.000000000000   )
    deposit_dict['quoteQty'] = 0
